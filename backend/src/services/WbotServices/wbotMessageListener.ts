...
import { wh_messages, wh_status } from "../../controllers/WebHookController";
...
const getTypeMessage = (msg: proto.IWebMessageInfo): string => {
	try {
		...
		if (type === "ephemeralMessage") {
			...
			if (msg?.message?.ephemeralMessage?.message?.videoMessage) {
				type = "videoMessage"
			}
...
export const getBodyMessage = async (msg: proto.IWebMessageInfo): Promise<string> => {
	...
	try {
		...
		if (!type) {
			logger.error("Não achou o type 90");
			return;
		}
...
const isValidMsg = (msg: proto.IWebMessageInfo): boolean => {
	...
	try {
		...
		logger.info(`Tipo: ${ifType}`)

		if (!ifType) {
			logger.warn(`#### Nao achou o type em isValidMsg: \n${msgType} ${JSON.stringify(msg?.message, undefined, 2)}`);
...
const wbotMessageListener = async (wbot: Session, whatsapp: Whatsapp, companyId: number | string): Promise<void> => {
	try {
		wbot.ev.on("messages.upsert", async (messageUpsert: ImessageUpsert) => {
			...
			if (!messages) return;
			//
			await wh_messages(wbot, whatsapp, messageUpsert);
			//
		...
		wbot.ev.on("messages.update", async (messageUpdate: WAMessageUpdate[]) => {
			...
			//
			await wh_status(wbot, whatsapp, messageUpdate);
			//
		...
		wbot.ev.on('message-receipt.update', async (events: any) => {
